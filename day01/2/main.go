...
func main() {
	...
	fmt.Println(lists.getSimularityScore())

	//fmt.Println(lists.totalDistance())
}

func (l *lists) getSimularityScore() int {
	simScore := 0

	for _, v := range l.list1 {
		if _, ok := l.occurenceList2[v]; ok {
			simScore += v * l.occurenceList2[v]
		}
	}
...
func readInput(fileName string) lists {
	...
	var list1 []int
	var list2 []int
	occuranceList2 := make(map[int]int)
	for scanner.Scan() {
		line := scanner.Text()
		lineSplit := strings.Split(line, "   ")
		list1 = append(list1, StringToIntWOError(lineSplit[0]))
		list2 = append(list2, StringToIntWOError(lineSplit[1]))
		occuranceList2[StringToIntWOError(lineSplit[1])]++
	}

	sort.Ints(list1)
	sort.Ints(list2)

	return lists{list1, list2, occuranceList2, nil}
}
